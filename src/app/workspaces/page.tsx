...
import { Building2, ArrowUpRight, Bot, Tag } from "lucide-react";
  ...
  return (
    <PageShell nav={{ active: "workspaces", crumb: "Workspaces" }}>
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <PageGreeting
          title="Workspaces"
          subtitle="Open a workspace to manage agents, CRM, and operations."
        />
        <Link href="/setup">
      ...
      {workspaces && workspaces.length > 0 && (
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {workspaces.map((ws) => {
            const activeAgents = ws.agents.filter((a) => a.status === "active").length;
            return (
                ...
                <Card hover className="group h-full p-6">
                  <div className="mb-5 flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h2 className="font-semibold text-ink-900">{ws.name}</h2>
