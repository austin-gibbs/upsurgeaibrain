...
//   npx tsx scripts/place-call.ts <agentId> <contactId> <toNumber> <attempt>
import { config as loadEnv } from "dotenv";
loadEnv({ path: ".env.local" });
import { placeCall } from "@/lib/engine/caller";

async function main() {
  const [agentId, contactId, toNumber, attempt] = process.argv.slice(2);
  if (!agentId || !contactId || !toNumber) {
    throw new Error("usage: tsx scripts/place-call.ts <agentId> <contactId> <toNumber> <attempt>");
  }
